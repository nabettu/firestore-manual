...
//image[cunstomfunction][カスタム関数の例][scale=0.5]{
//}
...
//image[selectbox][セレクトボックスをクリックした状態][scale=0.5]{
//}

その後セキュリティルールの初期設定をどうするか表示されますが、こちらは後ほど変更するためロックモードのまま次へ進みます。

//image[securityrule][セキュリティルールの選択][scale=0.5]{
//}
...
//image[rules][セキュリティルールの設定][scale=0.5]{
//}
...
//image[simurateBtn][シミュレータの実行][scale=0.5]{
//}
...
//image[auth_error][認証していないユーザーのアクセス][scale=0.5]{
//}
...
初版P45「5.5 メモの保存と読み込み機能の作成」をFirestoreに移行します。
...
管理画面でデータがFirestoreへ保存されていることが確認出来たら次は読み取りへ移ります。

//image[datasave][管理画面でのFirestoreのデータが保存されたことの確認][scale=0.5]{
//}
